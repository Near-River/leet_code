...
class Solution(object):
    def combinationSum(self, candidates, target):
        ...
        d = OrderedDict(sorted(d.items(), key=lambda x: x[1]))  # sorting dict by value
    ...
    def combinationSum2(self, candidates, target):
        ...
        def find_sums2(target, index, path):
            ...
            for i in range(index, len(candidates)):
                ...
                if prev != -1 and prev == candidates[i]: continue  # remove the repeated candidate number
                path.append(candidates[i])
                find_sums2(target=target - candidates[i], index=i, path=copy(path))
                path = path[:-1]
                prev = candidates[i]
